...
function getAddToCartItemWithCorrectQuantity(
  ...
  const sameItemInCart = orderFormItems.find(i => i.id === String(cartItem.id))

  if (!sameItemInCart) {
    return [cartItem]
  }

  const itemWithCorrectQuantity = {
    ...cartItem,
    quantity: cartItem.quantity + sameItemInCart.quantity,
  }

  return [itemWithCorrectQuantity]
}
...
function AddToCartButton(props: Props) {
  ...
  const handleAddToCart = async () => {
    ...
    const addToCartItems = getAddToCartItemWithCorrectQuantity(
      addToCartItem,
      items
    )
